    ...
    <div className="min-h-screen bg-[#0B0C10] text-white flex flex-col md:flex-row pb-24 md:pb-0 font-sans selection:bg-[#FF9F1C] selection:text-black">
      ...
      <aside className="w-full md:w-64 bg-[#1F2833] p-4 md:p-6 flex flex-row md:flex-col justify-between md:justify-start gap-4 fixed bottom-0 md:sticky md:top-0 md:h-screen z-50 border-t md:border-t-0 md:border-r border-gray-800 shadow-2xl shrink-0">
        ...
        <nav className="flex flex-row md:flex-col gap-2 w-full justify-around md:justify-start overflow-x-auto md:overflow-visible">
          {["dashboard", "membership", "rewards", "referral", "profile"].map((tab) => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 py-2.5 rounded-xl font-semibold text-sm min-w-max text-center transition-all capitalize w-full ${activeTab === tab ? "bg-[#FF9F1C] text-black shadow-md shadow-[#FF9F1C]/20" : "text-gray-400 hover:text-white hover:bg-gray-800/30"}`}
            >
              {tab}
            </button>
          ))}
          ...
          <div className="space-y-6 animate-fadeIn">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              ...
              <div className="bg-[#1F2833]/50 p-6 rounded-2xl border border-orange-500/10 flex flex-col justify-between gap-2">
                ...
                <p className="text-[11px] text-gray-500">Go to Rewards tab to process dynamic smart-contract claim.</p>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  {activeWalletTab === "receive" && (
                    <div className="bg-black/20 p-3 rounded-xl border border-gray-800 text-center space-y-2">
                      ...
                      <button 
                        type="button"
                        onClick={() => { navigator.clipboard.writeText(profile.wallet); alert("Address copied!"); }}
        ...
        {activeTab === "rewards" && (
          <div className="space-y-6 animate-fadeIn">
            <div>
              ...
              <p className="text-sm text-gray-400">Claim your split networks rewards immediately via smart execution.</p>
            </div>
        ...
        {activeTab === "profile" && (
          <div className="space-y-6 animate-fadeIn">
            <div>
              ...
              <p className="text-sm text-gray-400">Manage identity configuration strings and secure asset wallet alignment parameters.</p>
            </div>
              ...
              <div className="bg-[#1F2833]/40 border border-gray-800 rounded-2xl p-6 lg:col-span-2">
                ...
                <form onSubmit={handleSaveProfile} className="space-y-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-1">
                      <label className="text-[11px] text-gray-400 font-bold uppercase">Default Connected Node Wallet Address</label>
                      <input 
                        type="text" 
                  ...
                  <button type="submit" className="bg-[#FF9F1C] text-black font-extrabold px-6 py-2.5 rounded-xl text-xs shadow-md tracking-wider">
                    SAVE CHANGES
                  </button>
                </form>
              </div>
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 z-[100] animate-fadeIn">
          <div className="bg-[#1F2833] border border-gray-800 rounded-3xl p-6 w-full max-w-md space-y-4 relative shadow-2xl">
            <div>
              ...
              <p className="text-xs text-gray-400">Total Premium Cost Requirement: {selectedPlan.price}</p>
            </div>
            
            <form onSubmit={handleActivatePlanSubmit} className="space-y-4">
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-300">Your Sender Node Wallet Address</label>
                <input 
                  type="text" 
                  required
                  placeholder="0x..." 
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-gray-300">Blockchain Transaction Hash ID Proof</label>
                <input 
                  type="text" 
                  required
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setIsModalOpen(false)} className="flex-1 bg-gray-800 hover:bg-gray-700 text-white font-semibold py-2.5 rounded-xl text-sm transition-all">
                  Cancel
                </button>
                <button type="submit" className="flex-1 bg-[#FF9F1C] hover:bg-[#e08b14] text-black font-bold py-2.5 rounded-xl text-sm transition-all shadow-md shadow-[#FF9F1C]/10">
                  Submit Verification
                </button>
              </div>
            </form>
